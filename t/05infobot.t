...
is( $bot->tell_direct("search for foo"), "privmsg only, please", "not searched in public");
...
is( $bot->tell_direct("bar is yellow"), $but_reply,
  "Can't just redefine factoids" );
is( $bot->tell_indirect("bar is yellow"), '',
  "Can't just redefine factoids" );
is( $bot->tell_indirect("bar?"), "bar is green", "not changed" );
is( $bot->tell_direct("no, bar is yellow"), "Okay.", "Can explicitly redefine factoids" );
...
is( $bot->tell_direct("what is foo?"), "foo is foo", "English-language get" ); # fails
...
is( $bot->tell_direct("hoogas are things"), "Okay.", "simple 'are' set"); # fails
is( $bot->tell_direct("what are hoogas?"), "hoogas are things", "English-language 'are' get" );

is( $bot->tell_direct("foo is a silly thing"), "... but foo is foo ...", "warning about overwriting" );
...
is( $bot->tell_direct("foo is also | a silly thing"), "Okay.", "alternate appended");

is( $bot->tell_direct("literal foo?"), 
           "foo =is= foo =or= bar =or= baz =or= quux =or= |a silly thing", 
           "entire factoid looks right");
is( $bot->tell_direct("foo is also |<reply>this is a very silly thing"), "Okay.", "and a reply");
is( $bot->tell_direct("literal foo?"), 
